...
### 檢定力:F2 在固定 split 上量不出來

`truncated_by_full_frame` 是 613 支,但**落在 test split 的只有 90 支**。Stage B 在
244 支上的成對 bootstrap 半寬約 0.052,而 bootstrap CI 隨 1/√n 收斂:

| 子集 | 全體 | test | 外推 CI 半寬 |
| --- | --- | --- | --- |
| 全部 | 1623 | 244 | 0.052 |
| 非正方形 | 855 | 125 | 0.073 |
| **被 full_frame 截斷** | **613** | **90** | **0.086** |

(外推值,不是量測值。)整個 person-crop 效應本身只有 +0.026,而 primary 對比的
CI 半寬是 0.086。**這個對比在事前就注定不會顯著。** F1 的對比用滿 244 支、半寬
0.052,是四格裡唯一在 Stage B 檢定力下可讀的一個——而它已經量過了,就是
0.666 vs 0.640。

因此 B1 的 F2 部分有兩條路,**必須在跑 kernel 前挑定**:

1. **改用 repeated splits**(B2 power 節的選項 A 提前到 B1):對 1623 支做 5×5-fold,
   被截子集回到 613 支可評分,外推半寬約 0.033。代價是 pose-only 分母必須在同一組
   重抽上重算,且要同時報告固定 split 的數字以維持與 0.650 / 0.555 可比。
2. **不跑 F2**,在計畫裡寫明本語料庫回答不了這個問題,只留 F1。

### 事前登錄:repeated splits(選項 1,已採用)

在跑任何一臂之前寫定。

| 項目 | 值 |
| --- | --- |
| 重抽設計 | 5 repeats × 5-fold,stratified,**video-level** |
| 每折 | test = 1/5(約 325);val 由剩下的取 15%(約 195);train 約 1103 |
| 每個 repeat | 每支影片恰好被 test 一次 → 一個 repeat = 一組覆蓋全 1623 的 out-of-fold 預測 |
| repeats 取代 seeds | 訓練 seed 固定,折本身就是隨機化;兩者一起變會把 split 噪音和初始化噪音混進同一個數字 |
| 閾值 | 每折用自己的 val 選,套在自己的 test 切片上;**不**把機率池化後再選一個閾值 |
| Bootstrap | 抽影片,**同一個抽樣套用到全部 5 個 repeat**,再對 repeat 平均。25 折當獨立抽樣會讓區間窄約 √5 倍 |
| 折的種子 | 20260812,寫死;加 repeat 不會改動既有 repeat 的分割 |

**分母閘門(先跑,單獨跑)**:pose-only 在同一組折上重抽。

| 結果 | 判定 |
| --- | --- |
| 落在 0.650 ± 0.03 | 重抽與固定 split 可比,繼續 |
| **高於 0.650 + 0.03** | **停。** 每折只訓練約 1103 支(固定 split 是 1136),資料變少不可能讓分數上升,
唯一合理解釋是同一位受試者的多支影片同時落在兩邊——Fitness-AQA 沒有 participant mapping,
偵測不到。此時所有下游 delta 都被灌水 |
| 低於 0.650 − 0.03 | 重抽比固定 split 難;可繼續,但所有數字都要以新分母陳述 |

這個閘門只有一個方向可以解釋成好消息,所以它是能做的最強的洩漏檢查。

### 一個更乾淨但仍不夠的 F2 臂(已評估,未實作)

`full_frame_letterbox` 付 30% 相對面積是因為它補到完整正方形。有更省的做法:
**只補到人框剛好能通過 centre crop 為止**。這個候選臂的 ΔF3 中位數是 **−0.007**
(對比 letterbox 的 −0.122),乍看解決了 F3 混淆。

但逐支看,ΔF3 的 p10/中位/p90 是 **−0.101 / +0.012 / +0.051**——它在不同影片上
往兩個方向走,原因是 full_frame 的截斷有兩種成因,價錢不同:

| 成因 | 支數 | 佔截斷的 | 修法的代價 |
| --- | --- | --- | --- |
| **scale**:人框比 centre crop 視窗還高 | 314 | 51.2% | 只能縮小,必付 F3 |
| **framing**:人框放得下,但沒在畫面中央 | 299 | 48.8% | 只要把視窗對準受試者,**零代價** |

近一半的截斷根本不是解析度問題,而是 centre crop 對準了畫面中心而受試者不在那裡。
只取這 299 支可以得到 ΔF3 恰為 0 的完美 F2 對比——但它們在 test split 只有 **44 支**,
外推 CI 半寬 0.122。乾淨,但更量不出來。
...
但實作後的量測把這個期望值調低了兩次,而且第二次是決定性的:

一、centre crop 只動到 37.8% 的影片,在這些影片上中位只丟 12% 的框面積,而還原它
要付出 30% 的相對縮小。「預處理 bug」這條線的先驗比寫這份計畫時弱。

二、更要緊的是**檢定力**:F2 的對比在固定 split 上只有 90 支 test 影片,外推 CI 半寬
0.086,而要偵測的效應是 +0.026。**照原樣跑兩個 kernel,結論在跑之前就已經是「不確定」。**

所以修正後的建議是:**先決定 F2 要不要改用 repeated splits,再跑 kernel。** 兩個新臂
的程式碼已經就位,抽取只是幾小時的機器時間;但在固定 split 上跑它,得到的是一個
注定不顯著的數字。若不願承擔 repeated splits 的代價(分母重算、與歷史數字的可比性),
誠實的做法是只保留 F1、把 F2 記為本語料庫回答不了的問題,並在論文裡以此為限制陳述。

這個結論全部來自幾何運算,沒有用到任何 GPU 時間——這正是把取景量測放在抽取之前的
理由。
