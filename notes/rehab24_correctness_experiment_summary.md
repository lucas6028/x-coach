...
產物：`data/REHAB24-6/processed/correctness_loso_{vicon,mediapipe,mmpose,videomae}.json`（含每折明細）、`correctness_mmpose_fixed.json`（固定切分）、`correctness_loso_hrnet_vs_rtmpose.json`（HRNet-w48 vs RTMPose 配對對照）。執行：`python scripts/rehab24/loso_cross_validation.py --feature-dir <feature_dir>`；配對對照 `python scripts/rehab24/loso_hrnet_vs_rtmpose.py`。HRNet-w48 特徵在 Kaggle GPU 上抽取（見 `notebooks/rehab24_hrnet_colab.ipynb`，feature_dim=1188，2144 reps，與 RTMPose 2D-only 同特徵空間）。
...
| Ex | 動作 | n | pos% | Vicon | MediaPipe (pseudo-3D) | MMPose (2D-only) | HRNet-w48 (2D-only) | VideoMAE |
|---|---|---|---|---|---|---|---|---|
...
產物：`data/REHAB24-6/processed/correctness_loso_per_exercise.json`（4 來源）、`correctness_loso_per_exercise_hrnet.json`（HRNet-w48 vs RTMPose）。執行：`python scripts/rehab24/loso_per_exercise.py`（4 來源）；HRNet 對照 `python scripts/rehab24/loso_per_exercise.py --sources mmpose=mmpose_skeleton_features hrnet=hrnet_w48_skeleton_features --summary-output data/REHAB24-6/processed/correctness_loso_per_exercise_hrnet.json`。（RTMPose 欄位完全重現原表，確認折切分確定性、可逐欄並列。）
