...
「特徵 → within-class position」的線性方向，把它從所有樣本的特徵裡投影掉，再重新訓練分類器，
...
**第一句（機制，確定）。VideoMAE 的特徵確實線性編碼了 within-class position（這一下在同類 rep 裡排第幾）。**
...
**第二句（事前登錄的 primary，依 gate 規則不得判讀）。拿掉一個方向不夠。**
...
**第三句（事前登錄的 secondary，是唯一通過 positive control 的 arm）。拿掉 16 個方向後，AUC 0.8556 ± 0.0435，
