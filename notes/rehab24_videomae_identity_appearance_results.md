...
在位置資訊較弱的錄影裡（位置 AUC 落在 0.35–0.65），模型平均 AUC 0.7975；
在位置資訊強的錄影裡是 0.8774。但**位置中性的錄影只有 3 段、來自 2 位受試者**，
這個對比**沒有推論價值**，只能當敘述。

### 7.2 上界控制：把人塗掉，時間漂移留著

`background_only`（人被塗掉、場景與光線與所有隨時間的漂移都保留）跑完全相同的
predict → analyze 流程：
...
- **這是關鍵的上界**：如果 0.8741 主要來自「錄影中的時間位置經由場景／光線漂移洩漏」，
  那麼一個保留了全部場景漂移、只刪掉人的條件應該也要接近 0.87。它只有 0.5357。

所以「位置經由**非人物像素**洩漏」這條路徑被壓到約 0.54。
**但這不排除位置經由人物像素洩漏**——受試者在一段錄影中會疲勞、會位移、會改變站位，
這些都在人身上，`background_only` 量不到。這一層本資料集無法再分辨。
...
測試：`.venv\Scripts\python.exe -m pytest tests/test_rehab24_videomae_identity_control.py`（45 項）。
