...
48 條規則中有 8 條登錄後永不觸發，這是系統最明確的缺口。肩胛翼狀突出與聳肩代償量不到，原因不在規則寫法，而在單鏡頭取不回它們需要的離面資訊。補一個拍攝視角、多一部相機，或改用能取回離面深度的姿態來源，其中一部分就有機會由沉默轉為可判定；前提是每一次解除沉默都要附上該視角下的量測證據，否則只是拿一條不會觸發的規則換一條會亂觸發的規則。

其餘的缺口都要等標註資料補上。深蹲的規則比對必須在目前的產線重跑，並逐視角分層：表 2 的數字產生於逐次動作分段上線之前，全視角合併列量到的又是這批資料的視角分布而非規則品質，而側視角只有 138 支，穩定結論恐怕得先擴充側視角樣本；區段 IoU 0.045 則說明時間定位比是非判定更不可靠，需要獨立的定位指標。其餘 13 種登錄動作缺的是帶錯誤類型的人工標註，沒有它就只能檢定規則訊號與整體正確性是否相關，不能宣稱偵測到特定錯誤，Beta 標籤也撤不掉；已驗證這個旗標目前仍是人工判斷，要改成由標註集與回歸腳本撐腰。可運作不等於已具專家效度，這條界線要等外部標註到位才移得動。
