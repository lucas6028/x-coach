...
一次分析交回的不是一句評語，而是一份可以逐項回放的紀錄。系統為每一筆偵測到的錯誤記下錯誤代號與名稱、起訖影格與對應時間、嚴重度最高的峰值影格、它落在哪一個動作階段、嚴重度與信心、該規則在當前視角下的可觀察性等級，以及觸發它的原始量測與所用門檻，另附支持該規則的文獻出處與被引用的那一項發現。介面把這些欄位分別畫成同步骨架、錯誤時間軸與證據卡片，使用者看到的每一句話都追得回一個數字。
...
系統已為 16 種動作設計規則偵測器，其中 14 種登錄於執行時登錄表，合計 48 條規則實作。表 1 列出登錄的 14 種動作、各自的規則數與用來切分反覆的訊號。每一個偵測器都要宣告自己的量測集合、反覆訊號、訊號極性、起始狀態與階段切分函式，前端因而能直接由後端登錄表取得可分析的動作清單。

**表 1　登錄於執行時登錄表的 14 種動作偵測器。括號內為登錄但永不觸發的規則數。**

| 動作 | 規則數 | 反覆偵測訊號 | 狀態 |
| --- | ---: | --- | --- |
| 深蹲 | 5 | 平均膝關節角極小值 | 已驗證 |
| 肩上推舉 | 5 | 平均肘關節角極大值 | Beta |
| 伏地挺身 | 5（1） | 最小肘關節角極小值 | Beta |
| 弓箭步 | 4 | 最小膝關節角極小值 | Beta |
| 划船 | 4 | 最小肘關節角極小值 | Beta |
| 彈力帶擴胸 | 4（1） | 腕距（肩寬正規化）極大值 | Beta |
| 手臂 VW 開合 | 4（1） | 平均上臂仰角極小值 | Beta |
| 硬舉 | 3 | 髖關節角極小值 | Beta |
| 二頭彎舉 | 3 | 平均肘關節角極小值 | Beta |
| 手臂外展 | 3（1） | 平均上臂仰角極大值 | Beta |
| 仰臥起坐 | 2（1） | 髖關節角極小值 | Beta |
| 臀橋 | 2（1） | 髖關節角極大值 | Beta |
| 腿部外展 | 2（1） | 大腿與軀幹夾角極大值 | Beta |
| 軀幹旋轉 | 2（1） | 旋轉位移比極大值 | Beta |

48 條之中有 8 條登錄後永不觸發，而這與規則被撤回是兩件事。永不觸發指的是錯誤本身有文獻依據，單鏡頭的感測條件卻撐不起它——肩胛翼狀突出與聳肩代償都屬此類，量測不到並不表示它不存在，把規則登錄下來並讓它保持沉默，規格與程式才維持一對一對應，日後也不會有人把規則的缺席讀成該錯誤不存在。撤回則是連登錄都沒有：判準本身站不住，或是它在資料上被否證，這時留一個沉默的空殼反而是在宣稱一件未經證實的事。
...
**表 2　深蹲規則與 Fitness-AQA 標註的比對，n = 1,623，全視角合併。**

| 規則 | 骨架 | TP | FP | TN | FN | 精確率 | 召回率 | 特異度 | 平衡準確率 | 區段 IoU |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| knees_forward | MediaPipe | 1 | 0 | 514 | 1108 | 1.000 | 0.001 | 1.000 | 0.500 | 0.000 |
| knees_forward | RTMPose | 5 | 0 | 514 | 1104 | 1.000 | 0.005 | 1.000 | 0.502 | 0.001 |
| knees_inward | MediaPipe | 79 | 185 | 1206 | 153 | 0.299 | 0.341 | 0.867 | 0.604 | 0.045 |
| knees_inward | RTMPose | 116 | 397 | 994 | 116 | 0.226 | 0.500 | 0.715 | 0.607 | 0.055 |
...
**表 3　REHAB24-6 受試者留一交叉驗證，9 折平衡準確率。**

| 特徵設定 | Balanced accuracy | 判讀 |
| --- | ---: | --- |
| RTMPose 二維骨架 | 0.571 | 單鏡頭二維姿態 |
| MediaPipe 骨架 | 0.634 | 產線使用的單鏡頭基線 |
| NLF 三維骨架 | 0.668 | 本表最高 |
| 標籤置換虛無對照 | 0.497 | 機率水準 |
...
| 關係型態 | 數量 | 連接的內容 |
| --- | ---: | --- |
| AFFECTS_QUALITY | 862 | 錯誤或訊號影響哪一個品質面向 |
| INDICATED_BY | 713 | 錯誤由哪一項可量測訊號指示 |
| CAUSED_BY | 461 | 錯誤的成因 |
| INCREASES_RISK_OF | 326 | 錯誤提高的傷害風險 |
| HAS_PHASE | 297 | 動作包含的階段 |
| HAS_FAULT | 250 | 動作可能出現的錯誤 |
| OCCURS_IN_PHASE | 237 | 錯誤發生在哪一個階段 |
| CORRECTED_BY | 144 | 對應的修正提示 |

規則產生一個錯誤術語之後，由該錯誤節點出發即可走到它的成因、它提高的風險、它出現的階段與對應的修正提示，這正是回饋要說的四件事。文本檢索走另一條路，向量索引含 3,307 個文本區塊，回傳的是可引用的文獻段落。圖譜涵蓋 16 種動作，可檢索知識的動作因而多於可執行視覺分析的動作，介面與論文都必須維持這個差異。
...
最優先的工作由 4.3 節指定：深蹲的規則比對要在目前的產線上重跑，逐視角分層報告，並補上 shallow depth 那一類。分層是重點而非細節——knees_forward 只在側面判定，把它放進全視角合併列，量到的是視角分布而不是規則品質，先動門檻只會把一條規則調去迎合錯的分母。側視角在這批資料只有 138 支，要得到穩定結論恐怕還得補人工視角標註或擴充側視角樣本。區段 IoU 0.045 則指出另一個弱點：時間定位比是非判定更不可靠，這需要獨立的定位指標，不能靠影片層級的比對代答。

其餘 13 種動作則要逐一取得帶有錯誤類型的人工標註，否則只能判斷規則訊號與整體正確性是否相關，不能宣稱偵測了特定錯誤。弓箭步的候選前導腿量測應在獨立任務中替換，重跑所有規則，再以受試者層級的統計評估。受試者人數少的資料一律採 leave-one-subject-out、bootstrap 信賴區間或受試者層級置換檢定，不把同一人的多次反覆當成獨立樣本。深蹲資料另可依視角重新分析錯誤的可觀察性，建立 per-class、per-view 的混淆矩陣與錯誤案例集。

永不觸發的那 8 條規則指出感測條件的缺口在哪裡，值得逐條處理而不是擱置。肩胛翼狀突出、聳肩代償這類錯誤在單鏡頭下量不到，但換一個視角、加一部相機或改用能取回離面深度的姿態來源，其中一部分就有機會由沉默轉為可判定，前提是每一次解除沉默都要附上該視角下的量測證據，而不是把門檻直接開啟。

未標註深蹲影片可用於對比學習或動作週期自監督，前提是正負樣本的設計不會把人物、視角或背景當成捷徑：在受試者外觀明顯而樣本數有限的資料上，這類表徵很容易學到是誰在做，而不是做得如何。單鏡頭的離面深度仍是重要限制，4.5 節的 0.634 與 0.668 只量到差距的大小，沒有說明它落在哪些判準上。後續可比較更穩定的直接 image-to-3D 模型、多視角或深度感測，但指標應該是具體的錯誤決策有沒有改善，而不是平均關節位置誤差。

知識與生成層的驗證另有一套。以 RAGAS 的檢索相關性與忠實度指標 [9]，搭配具證照教練或運動專家的盲評，分別比較無檢索、純文本 RAG、知識圖譜檢索及雙軌檢索。專家評估要拆成觀察正確性、因果表述保守度、修正可操作性、引用支持度與潛在安全風險，而不是只評整體像不像教練。證據不足時，系統應該可以輸出無法由目前視角判定或請使用者重新拍攝，而不是為了把話說完而補上推測。

系統本身則朝非同步分析佇列、端側姿態推論、資料最小化與更完整的使用者研究發展。正式 TANET 定稿前，仍需補齊作者資訊、使用者或專家研究、RAG 忠實度實驗、各圖表的最終版面及倫理與資料使用聲明。現階段的結論是：以可解釋姿態證據作為判斷主體、以知識檢索作為生成約束，已形成可運作的 AI 運動教練原型。這條路線的代價也寫在系統自己的紀錄裡——48 條規則有 8 條因為單鏡頭量不到而保持沉默，14 種登錄動作只有一種有逐錯誤標註可以對照，而那一種的對照結果是一條規則可用但每十次觸發約七次是誤報、另一條連量它的分母都還沒對，弓箭步的前導腿判準則被自己的資料推翻。這些數字之所以留在論文裡，是因為它們與規則同時可見才構成一條可稽核的證據鏈；一個把判斷交給黑箱分數的系統，連讓這些數字浮現的位置都沒有。可運作不等於已具專家效度，系統是否安全、能否跨使用者、跨視角與跨動作泛化，仍須以更細緻的外部標註與專家評估證明。
