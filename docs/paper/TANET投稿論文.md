...
**圖 2　系統資料流。** 系統主路徑先形成可檢查的姿態與規則證據，再啟動知識檢索與生成。錯誤判定不經過語言模型，語言模型只在流程末端負責解釋與互動。
...
深蹲規則的比對以 Fitness-AQA 的 1,623 支標註影片為對象。規則在每支影片上重播，產生的錯誤區間與標註區間對齊之後，逐類計算真陽性、假陽性、精確率（precision）、召回率（recall）、特異度（specificity）與區段 IoU（segment IoU，偵測區間與標註區間的交集除以聯集）；同一組規則另在 RTMPose 骨架上重播一次，用來分辨結果是規則造成的還是姿態器造成的。判定以影片為單位，因此度量的是規則會不會在這支影片上出現，而不是它在每一次反覆上的表現。姿態模組的另一項對照直接比較兩個 MediaPipe 複雜度設定：同一批影片分別以 Lite 與 Heavy 擷取姿態，送進同一組規則，比較兩者產生的錯誤集合是否完全相同。

骨架來源的比較在 REHAB24-6 上採受試者留一交叉驗證，九位受試者輪流留出，比較單鏡頭二維骨架與直接由影像估計的三維骨架。虛無對照以標籤置換建立，置換在受試者內以反覆動作為單位執行，各受試者的正例率不變，機率水準因而未被更動。

正類偏多是這批資料共同的形狀，指標選擇必須據此設定。深蹲資料上的 always-positive 基準，正類 F1 約為 0.83；前置實驗中以該資料訓練的分類器，有的同樣取得約 0.83 的 F1，特異度卻低至約 0.04。只報正類 F1 會把一個從不說不的模型描述成表現良好，主要指標因而取平衡準確率（balanced accuracy，正負兩類各自正確率的平均）與特異度，並把恆正與恆負兩個基準一併納入比較。
...
深蹲的五條規則中有三條對得上 Fitness-AQA 的錯誤標註。規則在 1,623 支標註影片上重播，偵測區間與標註區間對齊後逐類統計，表 2 為當次回報的兩類，並以同一組規則換到 RTMPose 骨架作為對照。
...
knees_inward 是這批資料上唯一可評估的規則。平衡準確率 0.604、特異度 0.867，精確率卻只有 0.299，每觸發十次約七次是誤報；區段 IoU 0.045 另顯示時間定位比是非判定更不可靠。knees_forward 的近乎零召回則不是門檻太嚴：它只在側面視角判定，其餘視角回傳的是無法判定而非沒有錯誤，而本資料的側面僅佔 8.5%，全視角合併因此把不該計分的影片算成漏報。這一列量到的是視角分布，重跑必須逐視角分層。換骨架也不解決問題，改用 RTMPose 後召回率由 0.341 升到 0.500，誤報同時由 185 升到 397，平衡準確率幾乎不動，可見門檻與姿態估計器綁在一起。

這次比對早於逐次動作分段導入，數字對應的是整段影片評估，shallow depth 亦未在當次回報，因此不以它描述目前流程的效能。它界定的是已驗證一詞的範圍：深蹲是唯一有標註可對照的動作，而對照本身就指出了兩處待修的缺口。

### 4.4 判準被自己的資料推翻：弓箭步稽核

弓箭步稽核展示了規則式方法的另一項價值：規則不只可解釋，也可以被否證。原先系統以底部時較彎曲的膝判斷前導腿，在 174 次反覆中，此方法於兩個相機的正確率僅 0.623 與 0.474。資料集的標記式三維骨架顯示這個假設本身就接近機率水準，並非單純受到二維投影影響。改以腳部前後位置建立的候選量測，在兩個單鏡頭視角則達 0.959 與 0.894。候選量測尚未完成替換與全規則重跑，本研究因此沒有調整門檻，弓箭步偵測器也持續標記為 Beta。固定規則的可信度來自可重播、可比較與可撤回的證據鏈，而不是規則看起來符合直覺。
...
姿態來源還有更上游的一層。表 3 為 REHAB24-6 受試者留一交叉驗證的結果，比較的是同一套下游流程接不同骨架：直接由影像估計的 NLF 三維骨架為 0.668，系統使用的 MediaPipe 骨架為 0.634，RTMPose 二維骨架為 0.571，標籤置換的虛無對照落在 0.497。

**表 3　REHAB24-6 受試者留一交叉驗證，9 折平衡準確率。MediaPipe 為系統實際使用的骨架，最末列為機率水準。**
...
系統最明確的缺口是 8 條寫進系統卻永不輸出判定的規則。以肩胛翼狀突出與聳肩代償為例，問題不在規則寫法，而在單鏡頭取不回它們需要的資訊：前者要看的肩胛骨在姿態模型的關鍵點裡並不存在，後者取得到的肩點是盂肱關節而非肩峰。多一個拍攝角度、多一部相機，或改用能取回離面深度的姿態估計，其中一部分有機會由沉默轉為可判定。前提是每一次解除沉默都附上該視角下的量測證據；只把門檻打開，等於拿一條不會觸發的規則換一條會亂觸發的規則。

深蹲的規則比對需要在現行流程上重跑。表 2 的比對執行於導入逐次動作分段之前，當時規則以整段影片為單位評估，也未涵蓋深度不足這一類。重跑時應依視角分層：膝蓋前移只在側面視角判定，把它放進全視角合併的數字，量到的是資料的視角分布而非規則品質，此時先調門檻等於讓規則去迎合錯誤的分母。這批資料的側面樣本僅 138 支，要得到穩定結論可能需要補充人工視角標註或擴充側面樣本。區段 IoU 0.045 指向另一項弱點：錯誤發生在哪一段的判斷比有無錯誤更不可靠，需要專門的定位指標，影片層級的比對無法代答。

其餘 13 種動作缺的是帶有錯誤類型的人工標註。沒有它，可檢定的只有規則訊號與整體正確與否是否相關，無法宣稱偵測到特定錯誤，Beta 標記也就撤不掉。開合跳與高抬腿未納入系統，正是這種標註存在時會發生的事：判準一旦對得上資料，就可能不通過。弓箭步的前導腿量測應先完成替換再重跑全部規則。受試者人數少的資料一律採受試者留一交叉驗證，並以受試者層級的區間估計或置換檢定推論，不把同一人的多次反覆當成獨立樣本。一個動作算不算已驗證，目前仍由人工判斷，應改由標註資料與可重複執行的評估決定。知識層的回饋是否忠實於檢索到的來源，同樣尚未驗證，需要檢索品質指標與領域專家評閱一併進行。

這兩項缺口共用同一個判準：規則能不能被外部證據推翻。沉默的 8 條指出感測條件撐不起哪些判準，待驗證的 13 種指出證據尚未到位，而弓箭步的稽核證明證據到位時判準真的會被推翻。這些數字留在論文裡，是因為它們與規則同時可見才構成一條可稽核的證據鏈；把判斷交給單一分數的系統，連讓它們浮現的位置都沒有。可運作不等於已具專家效度。
