...
knees_inward 是這批資料上唯一可評估的規則。平衡準確率 0.604、特異度 0.867，精確率卻只有 0.299，每觸發十次約七次是誤報；區段 IoU 0.045 進一步顯示時間定位比是非判定更不可靠。knees_forward 的近乎零召回則不是門檻太嚴：它只在側面視角判定，其餘視角回傳的是無法判定而非沒有錯誤，而本資料的側面僅佔 8.5%，全視角合併因此把不該計分的影片算成漏報。這一列量到的是視角分布，重跑必須逐視角分層。換骨架也不解決問題，改用 RTMPose 後召回率由 0.341 升到 0.500，誤報同時由 185 升到 397，平衡準確率幾乎不動，可見門檻與姿態估計器綁在一起。

這次比對早於現行的逐次動作分段，數字對應的是整段影片評估，shallow depth 亦未在當次回報，因此不以它描述目前產線的效能。它界定的是已驗證一詞的範圍：深蹲是唯一有標註可對照的動作，而對照本身就指出了兩處待修的缺口。
