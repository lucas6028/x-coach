...
| Red-flag rules (v1, deterministic) | (a) `pain_nrs ≥ 6`; (b) pain up by ≥ 3 vs the previous check-in on the same plan; (c) evaluated only when the new check-in HAS a form score: the mean of the last 3 **scored** check-ins on the plan (the new one included) is ≥ 20 below the mean of the 3 scored check-ins before them; fewer than 6 scored check-ins → no flag (form_score is null for an unmeasured clip or no analysis, so "last 3 sessions" cannot mean the last 3 rows); (d) 7 days without a check-in on the current assigned plan, computed on read for the dashboard (`inactive_7d`, WP1). | Explainable, defensible in Q&A, no model. |
| Check-in prompt | Opens automatically after a studio tick only when the plan is **assigned** (`assigned_by` set); every plan item row has a manual 「回報今天狀況」. Skippable. | Self-serve users' existing flow is unchanged; the care loop gets the prompt where someone reads it. |
| Patient-side routes | `/api/care/*` (accept code, list/unlink clinicians); `/api/clinic/*` is clinician-only except `/status`. | The WP1 acceptance "403 on every /api/clinic/* except status" cannot hold if a patient accepts under /api/clinic. |
