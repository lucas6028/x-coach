...
- **這次驗證真正沒過關的是前導腿的替代判準,而不是任何一個門檻——而且它框住了全部四條規則,**
  ...
  **但這個失敗首先是**量測**的失敗,其次才輪得到解剖學;因此文件刻意寫得比「彎曲較多無法辨識
  ...
  多」在 **33%** 的動作上彼此不一致(獨立重算得到 37%)——同一具身體同一瞬間,這只能是量測
...
- **`lunge_knee_valgus`——分離度弱但真實存在,也是四條規則裡最不受前導腿失準拖累的一條。**
  逐受試者 AUC 中位數 **0.590**(排除 40 下 level-2/3 有他人入鏡的動作後為 0.629;改用標註
  前導腿為 0.620)。事前預測的「步伐深度污染」**確實存在,但比預期弱**:只取*正確*的動作,
