...
- **這次驗證真正沒過關的是前導腿的替代判準,而不是任何一個門檻——而且它框住了全部四條規則,**
  ...
  判不出來。問題出在*前提*,不在程式:標註的前導腿在該下動作最低點真的是彎曲較多的那隻腳,
  只佔 **105/169 = 62.1%**(cam17)與 **78/161 = 48.4%**(cam18),而且左右膝角度中位數差距
  高達 **25 度**——所以 `LEAD_SIDE_MIN_SEPARATION_DEG = 5.0` 這道守衛根本攔不到。
  `resolve_lead_side` 的 docstring 早已載明:它用「彎曲較多」取代了本規格「彎曲較多 /
  **較靠前**的腳」這個定義的後半,因為前後軸在正面視角會塌掉;壞掉的正是被丟掉的那一半。
  弓步時兩隻膝蓋都會彎到大約直角,而後腳的膝蓋往地面沉,往往彎得更多。§7 的風險表明確
  預言過這個結果,而它成真了。**如實記錄,不就地修補**——換一個判準是偵測器層級的改動,
  要走它自己的驗證,不是在寫報告時順手改掉。
- **`lunge_knee_past_toes`——線索本身有資訊量,但目前出貨的規則拿不到。** 在 88 下真正矢狀面
  ...
  指標,逐受試者中位數是 **0.833**(全部 174 下則為 0.725)。閘門不是問題所在:視角估計器在
...
- **`lunge_insufficient_depth`——這個資料集沒有考到它,而它看起來的訊號其實是選擇偏誤。**
  ...
  分層看似有 0.792 的逐受試者 AUC,**改用標註前導腿後掉到 0.320**:`resolve_lead_side`
...
- **`lunge_pelvic_drop`——同樣沒被考到,而且事前預測的失效模式**沒有**發生。** §6.5 預測
  分腿站姿的前後縮短會在做得深又正確的動作上造成偽陽性。實測的正式流程特異度是整體 **0.923**、
  在 half-profile 分層上 **1.000**。但它在 174 下裡只觸發 **10 次**(4 真陽 / 6 偽陽),
  改用標註前導腿後停在 **0.467——隨機**。依計畫自訂的判準,**兩類標籤上**觸發率都接近零,
  結論是「這個資料集沒考到它」,**不是**「這條規則有效」。
