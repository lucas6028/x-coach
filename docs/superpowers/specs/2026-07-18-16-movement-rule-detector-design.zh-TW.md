...
**狀態(2026-07-25)——伏地挺身偵測器已註冊:** `src/pose/movements/pushup.py` 已組裝為
`PUSHUP_DETECTOR` 並以 `"Push-up"` 之名註冊,可由
`scripts/pose/run_pose_rule_detection.py --movement "Push-up"` 取用。伏地挺身的 5 條規則
**全數(5/5)** 到位:**4 條可觸發**(`pushup_hip_sag`、`pushup_shallow_depth`、
`pushup_head_drop`、`pushup_elbow_flare`),**1 條依設計永久沉默**(`rule_scapular_winging`
恆回傳 `[]`——MediaPipe 的 33 個關鍵點裡沒有任何肩胛骨邊界點,本規格也把該錯誤的可觀察度評為
`low`/`none`,並建議不要輸出有信心的判定。之所以註冊而非略過,是為了讓規格與程式維持 1:1
對應:稽核者會在這裡找到它與它的引用文獻,而不是找到一個缺口、然後用自己發明的代理指標把它補上)。

> **伏地挺身的門檻由規格推導、尚未驗證(§8.4)。** 本儲存庫 **完全沒有已標註的伏地挺身資料**
> ——REHAB24-6 的 Ex3 是 *站姿推牆/推桌* 而非本偵測器所建模的地板伏地挺身,EgoExo 的伏地挺身
> 影格則是一份尚未解壓的約 3 GB 壓縮檔。這個偵測器裡沒有任何一項曾對照人工標註的伏地挺身錯誤
> 檢驗過。`tests/test_pushup.py` 的合成 fixture 驗證的是幾何與正負號慣例,不是真實世界的錯誤
> 偵測能力。因此伏地挺身 **僅限 CLI**:`backend/app/config.py` 的 `DEFAULT_ANALYSIS_MOVEMENT`
> 維持 `"Squat"`,`frontend/src/lib/movements.ts` 的 `ANALYZABLE_MOVEMENTS` 也仍只有
> `["Squat"]`,這些數字不會傳到終端使用者面前。

實作與上文偵測啟發式的偏離,全部是刻意為之,程式碼內亦已註明:

- `pushup_shallow_depth`——以 **100 度** 觸發,取規格「約 100–110 度」區間中最保守(最不容易
  觸發)的一端,沒有使用區間外的任何數字。它讀的是 `min_elbow_angle`(較彎的那隻手臂),而規格
  寫的是「取較清楚可見的那側」;在左右不對稱的動作上,較彎的一側是較寬容的讀數,所以這條規則
  是少報而非多報。
- `pushup_head_drop`——**兩條軸都改用每支影片自己的 `setup` 基準線,而非絕對讀數。** 規格的
  措辭是絕對的,卻沒有(也無法)提供「中立姿勢」的絕對參考:耳到肩向量的靜止角度會隨頸長、
  耳位與攝影機高度而變。鼻部那條軸的需求更是可實測而非可爭論的:在中立 fixture 上量到
  `nose_ahead_ratio = 0.1833`,當下並沒有任何錯誤存在(是絕對門檻 0.06 的 3 倍),所以照字面
  絕對解讀,這條線索會在每一支正確的伏地挺身上觸發。因此本規則改測「相對於該影片自身 `setup`
  影格平均值的偏移量」,做法沿用本規格自己為深蹲腳跟離地所寫的建構方式。**誠實的代價:**
  它量的是 *變化* 而非 *姿勢*——一開始就把頭低著、並且一路維持的人永遠不會被標記(由
  `test_a_head_drop_held_from_setup_is_invisible` 釘住)。
- `pushup_head_drop`——鼻部線索的 **0.06 個身長是規則層的選擇,不是本規格的數字**。規格把這條
  線索以 OR 併入,卻只用「明顯在前方」來描述,沒有給出量值。這個量級是 **類比** 自
  `pushup_hip_sag` 規格內的「> 約 0.06 個身長」而來——這是對「明顯」的一個站得住腳的讀法,但
  終究是類比,日後不得被當成規格出處引用。
- `pushup_head_drop`——為支撐這條規則,指標層新增了 **帶正負號** 的
  `neck_line_signed_deg`。在無正負號的角度上做基準線,不只是失去方向性,而是 *方向被反轉*:
  基準線 +5 度時,把頭 **抬** 到 −15 度會算出偏移 +10,反而以「低頭」觸發。此外頸角改以
  **身體軸線**(肩中點 → 踝中點)為參考,而非規格所寫的肩→髖弦線,因為髖部下沉時弦線會轉動,
  等於用髖部塌陷製造出頸部偏移(實測:`hip_offset_ratio` 0.100 → 11.31 度的假訊號,相當於一次
  完整低頭訊號的 55%)。這個替換是把一個建模假設換成另一個,殘留的假設已在程式碼內載明,並未
  被「修正」掉。
- `pushup_head_drop`——階段範圍限定在 `{descent, bottom}`(規格未限定任何階段),因此只在
  回程才低頭的情況會被漏掉。此點是明講,不是藏起來。
- `pushup_elbow_flare`——**以可量測性閘門取代規格的視角閘門。** 規格要求從身體長軸方向的
  `front`/`rear` 視角觀察。Task 4 確認 `signed_orientation` 的前/後語意只在 *直立* 受試者上
  得到驗證,對水平身體而言那些標籤沒有經過驗證的意義;而生產路徑呼叫
  `estimate_view_for_pose(allow_front=False)`,`front` 根本不會被輸出——對這些標籤做正向閘門
  不是沒有意義就是永遠為假。本規則改為要求 `shoulder_axis_ratio > 0.15`(**規則層** 的可量測性
  門檻,不在本規格內:投影後仍保留足夠的橫向延展,手寬這個問題才答得出來),並在 *有信心的*
  `side` 標籤上硬性閘掉——「攝影機是否確實垂直於身體長軸」是答得出來的否定式宣稱。少了這道
  防護,矢狀面塌陷會偽造出判定:實測肩距 0.0020 對腕距 0.0050(兩者都在次像素等級、都沒有意義)
  會得出 `hand_width_ratio = 2.500`,亦即從純雜訊裡長出一個滿嚴重度的手肘外開判定。
- `pushup_elbow_flare`——一律以 **可觀察度 `low` 加上 0.65 信心折扣** 輸出,低於本規格對該
  錯誤所訂的 `medium` 上限,因為那個上限掛在對水平身體並未驗證的 `front`/`rear` 標籤上。
  `run_detector` 的排序把 `low` 的偵測排在所有其他偵測之後,與嚴重度無關(排序鍵為
  `(observability == "low", −severity, start_frame)`,而 `False < True`),所以手肘外開的判定
  永遠不可能排在「由管線真正驗證過的視角所觀察到的錯誤」之前。本規則的第二道閘門
  `hand_width_ratio > 0.25` 在觸發門檻仍為 1.6 的情況下 **在算術上是惰性的**,保留為顯式條件
  只是為了讓可量測性條件保持可讀。
- `pushup_elbow_flare`——規格中選用性的佐證訊號(軀幹對上臂夾角 > 約 65 度)**未實作**;
  指標層沒有輸出這個角度。
- `pushup_hip_sag` 與 `pushup_elbow_flare`——兩者的階段範圍都是 `{descent, bottom, ascent}`,
  這是沿用深蹲偵測器 `ACTIVE_PHASES` 的規則層決定;本規格對兩者都未限定階段。
- `pushup_hip_sag`——對 `front`/`rear` 採 **硬性沉默** 而非信心折扣,因為正對身體時位移的
  正規化分母會縮短、使讀數 *膨脹*,那是偽陽性放大器而不只是弱訊號。它另外要求
  `hand_offset_ratio > 0.0` 作為 **攝影機翻轉防護**:手掌撐在地面上,所以該值為負代表「朝地
  方向」被解錯了,屆時每一次髖部下沉都會被回報成有信心的髖部上頂。規格提到的髖角形式
  (「肩-髖-踝夾角偏離 180 度超過約 12 度」)只作為佐證寫入 evidence,**不** 參與觸發判定,
  因為它沒有正負號,一旦納入就會重新引入這道防護所要防的「無方向判定」。
- **伏地挺身所有規則的嚴重度斜坡都是規則層選擇,不是規格量值。** 本規格的伏地挺身章節完全
  沒有 `Severity ramp` 這一行(深蹲/弓步/硬舉章節有,所以這個缺席是有意義的)。所選的端點
  ——髖位移 0.15、肘角 140 度、頸角 35 度、鼻部 0.15、手寬 2.2——是排序用的曲線,程式碼內附有
  理由,但不是有出處的量值。

**另一項值得記錄、但不算偏離的模組層行為:** `pushup_compute_raw` 的 `required` 同時包含雙肩、
雙肘、雙腕、雙髖 **與雙踝**,因為平板線是肩中點 → 踝中點。只要掉了其中一個關鍵點,該影格即
判為無效,並同時讓 *所有* 伏地挺身規則在該影格沉默;因此一支只拍到膝蓋以上的影片根本不會產生
任何伏地挺身判定。這個「拒答」是刻意的——`view_estimation` 那種降級作法實測會把身體軸長讀成
0.070 而非約 0.60(低了 8.6 倍),既沒有 NaN 也沒有其他訊號,而一個悄悄算錯的判定比沒有判定
更糟。附帶一提,規格稱為主要視角的矢狀面,同時也是最容易踩到這條的視角。
...
**狀態(2026-07-30)—— 弓步已對 REHAB24-6 Ex5 完成驗證。這是本儲存庫**第一個**以人工標註的
真實答案檢驗過的動作,也就此為弓步(且僅為弓步)結清 §8.4。** 四條規則全數以
`scripts/rehab24/validate_lunge_rules.py` 重放過 **174 下標註過的動作**(8 位受試者、96 下
錯誤 / 78 下正確、兩台正交攝影機)。完整數字、方法與但書都在
**`notes/lunge-rule-validation.md`**;引用底下任何一個數字之前,請先讀那份文件。

**這批標註撐得起什麼、撐不起什麼。** REHAB24-6 只說一下動作正確或錯誤,**從不指名是哪個
錯誤**,所以規則在一下錯誤動作上觸發,並不等於它找到了那下動作真正的問題。以下所有數字量的
都是一件事:某條規則的訊號**是否帶有「這下動作正不正確」的資訊**——不是逐項錯誤的準確率。
主要統計量一律是**逐受試者**的(8 位裡有 7 位兩類標籤都有,取中位數與範圍;第 3 號受試者
21 下全錯、0 下正確,無法算 AUC);彙總數字只是次要,而且**不對彙總後的動作次數計算 p 值**。
在這裡分得開的東西,是**在這個資料集上**分得開——固定機位、受控光線、指定式錯誤的實驗室錄影。

**沒有任何門檻或嚴重度斜坡因為這些結果而更動,`LUNGE_DETECTOR.validated` 也仍然是 `False`。**
把有引用來源的數字調去迎合量測結果,那個引用就變成假的來源宣稱;而翻動 `validated` 是一項
「已對標註真值檢驗過」的產品宣稱,該由使用者拿著這些數字自己決定。

- **這次驗證真正沒過關的是前導腿的替代判準,而不是任何一個門檻——而且它框住了全部四條規則,**
  因為四條規則讀的都是 `f"{lead}_..."`。`resolve_lead_side` 與 `exercise_subtype` 相符的比例
  是 cam17 **96/154 = 0.623**、cam18 **72/152 = 0.474**(**低於隨機**),另有 11.5% / 12.6%
  判不出來。壞掉的是它拿來取代本規格「彎曲較多 / **較靠前**的腳」的那個*前提*(前後軸在正面
  視角會塌掉,所以只留下「彎曲較多」那一半):標註的前導腿在該下動作最低點真的是彎曲較多的
  那隻腳,只佔 **101/169 = 59.8%**(cam17)與 **77/161 = 47.8%**(cam18)——這是在**完整標註
  視窗**上量的,`segment_reps` 與平滑都不參與。在它判錯的那些動作上,左右膝角度差距中位數是
  19.4 度 / 25.4 度,遠遠超出 `LEAD_SIDE_MIN_SEPARATION_DEG` 那道 5 度的帶,所以守衛攔不到。
  而且守衛實際出手的次數比「判不出來」的比例更低:用 `lead_unresolved_reason` 拆開來看,真正
  屬於 5 度守衛的只有 cam17 **15/174 = 8.6%**、cam18 **9/174 = 5.2%**;其餘是資料缺失(沒有
  任何有效影格帶得出有限的 `min_knee_angle`:5 下與 13 下;最低點影格膝角非有限值:0 下與 0 下)。
  **但這個失敗首先是**量測**的失敗,其次才輪得到解剖學;因此文件刻意寫得比「彎曲較多無法辨識
  弓步的前導腿」更窄。** 三個對照如此顯示:兩台**同時**拍攝的攝影機,對於「哪隻膝蓋彎得比較
  多」在 **33%** 的動作上彼此不一致——同一具身體同一瞬間,這只能是量測
  誤差,不是解剖;兩個前提比率在這兩個同時視角之間差了 12 個百分點;而把同一個角度改成只在
  影像平面上計算、丟掉 MediaPipe 的偽深度 `z`,cam17 會從 **59.8% 掉到 17.2%**,cam18 反而
  往上走。(這兩個對照都在兩種取幀母體上各報一次:被 `segment_reps` 重切過的計分視窗,以及
  完整標註視窗。幾何完全相同,差的只是取哪一幀,而工具兩種都算:計分視窗版本讀出來是
  **58/156 = 37.2%** 與 **24/169 = 14.2%**。**引用的是完整視窗那一組,因為它與分段無關**——
  而這正是本論證不該向工具自己的切窗方式借來的那個性質;而且每一項結論在兩種母體上都成立。)因此站得住的說法是:**「彎曲較多這條線索,以本管線量測它的方式,
  在此處可得的任一視角下都無法辨識標註的前導腿」**——這仍然完整地判了出貨版 `resolve_lead_side`
  死刑,也仍然框住四條規則。**這個區分決定了後續怎麼修:這批資料無法區分「前提是錯的」與
  「前提在這個投影下取不回來」,所以該先試的是對深度穩健的前導腿線索,而不是換一條線索。**
  §7 的風險表明確預言過這個結果,而它成真了。**如實記錄,不就地修補**——換判準是偵測器層級的
  改動,要走它自己的驗證。
- **`lunge_knee_past_toes`——線索本身有資訊量,但目前出貨的規則拿不到。** 在 88 下真正矢狀面
  的 cam18 動作上,出貨版規則的指標是**反向的**:逐受試者 AUC 中位數 **0.171**(正確的動作被
  排在錯誤的動作*之上*)。改用 `exercise_subtype` 指定的那隻腳,在完全相同的影格上讀同一個
  指標,逐受試者中位數是 **0.833**(全部 174 下則為 0.725);**若只取兩種前導腿選法都有分數的
  那 80 下,對比是 0.171 對 0.850**,所以這不是分母造成的假象。(文件裡每一個 AUC 都附上它的
  動作數;兩欄各自因為前導腿判不出來而掉掉不同的動作。)`half-profile` 分層正式流程的 0.850
  不是反例:它的標註前導腿版本是 0.845,也就是在那個分層上選哪隻腳幾乎沒差,所以讓矢狀面動作
  反轉的那筆「選錯腳」代價,在那裡根本沒有被收取。閘門不是問題所在:視角估計器在
  這 88 下全部回報 `side`,與第 0 階段的 88/88 一致;而正式流程與 oracle 兩趟在此完全相同,
  是因為兩者在該分層之外都不會產生 `side`。
- **`lunge_knee_valgus`——分離度弱、只是**中位數**高於隨機,也是四條規則裡最不受前導腿失準
  拖累的一條。這不算通過驗證。** 逐受試者 AUC 中位數 **0.590**(排除 40 下 level-2/3 有他人
  入鏡的動作後為 0.629;改用標註前導腿為 0.620)——但七位受試者各自的值是 0.263、0.374、
  0.486、0.590、0.629、0.810、0.852,也就是**只有 4/7 位高於 0.5**,其中一位還明顯反向,
  而且**沒有做任何虛無假設檢定**(本工具不算置換檢定、不算信賴區間、不算顯著性;而基於獨立性
  的理由,這批動作本來就不報 p 值)。能講的只有「中位數在 7 位裡有 4 位高於隨機」,不是
  「已經排除隨機」。事前預測的「步伐深度污染」**確實存在,但比預期弱**:只取*正確*的動作,
  外翻代理量與最低點前導膝角度的 Spearman ρ = **−0.325**(錯誤動作上為 −0.211)——方向與
  形狀都如預測,但關聯性偏弱,所以步伐深度只解釋了部分觸發,而非大部分。在 86 下 `half-profile`
  動作上,這條規則觸發了其中的 **83%**(門檻落在該分佈的第 6.3 百分位),因此該分層 0.915 的
  敏感度毫無意義,**該看的主要數字是觸發率**。
- **`lunge_insufficient_depth`——這個資料集沒有考到它,而它看起來的訊號其實是選擇偏誤。**
  174 下裡只觸發 **6 次**(100 度的門檻落在觀測分佈的第 84.5 百分位——正是 `rank_auc`
  docstring 寫的那個情境:線索有資訊量,但被引用的切點落在分佈尾巴;而該切點不動)。矢狀面
  分層看似有 0.792 的逐受試者 AUC,**改用標註前導腿後掉到 0.320**(配對 n=80 時為 0.792 對 0.300):`resolve_lead_side`
  按定義挑的就是*彎曲較多*的那隻膝蓋,所以「被選中那隻膝蓋的最大角度」是對兩隻腳做的有偏
  統計量,而不是對前導腿的量測。改讀標註的那隻腳,整體只有 0.390,落在隨機水準或以下。這
  不代表規則是錯的——是代表這個錯誤在 REHAB24-6 的指定式錯誤裡不存在,或量不到。
- **`lunge_pelvic_drop`——幾乎沒被考到,而且 §6.5 預測的失效模式**並未被推翻**。** 它在 174 下
  裡只觸發 **10 次**(4 真陽 / 6 偽陽),改用標註前導腿後停在 **0.467——隨機**;依計畫自訂的
  判準,**兩類標籤上**觸發率都接近零,結論是「這個資料集沒考到它」,**不是**「這條規則有效」。
  至於 §6.5 提的那個風險(分腿站姿的前後縮短,會在做得深又正確的動作上造成偽陽性),看起來
  安心的那些數字全是假象,不能拿來把風險結案:half-profile 分層的 **1.000 特異度是空的**——
  該規則在那裡一次都沒觸發,因為下一點講的視角錯標把它整個關掉了,涵蓋全部 39 下正確動作,
  而「一條被關掉的規則的特異度」什麼都沒量到;整體那個 **0.923,則是把 78 下正確動作中的 54 下
  算成真陰,而那 54 下正是規則結構性沉默的那些**(37 下視角關閉 OR 18 下根本無法觸發,重疊
  1 下——聯集是 54 而不是 55)。只看**規則真的能動作的 24 下正確
  動作,它誤觸發了 6 下:特異度 0.750,偽陽率 25%**——這正是 §6.5 所預測的形狀,而不是它的
  反證。排除 40 下 level-2/3 有他人入鏡的動作後,該條件特異度變成 0.895、逐受試者 AUC 中位數
  變成 0.679,所以肇因不是 MediaPipe 認錯人。門檻落在觀測分佈的第 54.4 百分位(front 64.3 /
  half-profile 45.6)。**§6.5 的風險維持開啟**:6 次偽陽沒有任何檢定力去確認前後縮短這個機制。
- **正式流程與 oracle 之間唯一真正的落差是**閘門**失效,而且它修正了第 0 階段的結論。**
  `lunge_pelvic_drop` 在正式流程觸發 10 次、oracle 觸發 41 次,原因是視角估計器把 cam17
  的 **86 下 `half-profile` 動作中的 84 下判成 `side`**,而該規則在 `side` 下正確地回傳 `[]`。
  錯的是那個*標籤*。所以 `side` 這道閘門的敏感度不錯(在真正矢狀面的 cam18 動作上 88/88,見
  `notes/lunge-view-reconnaissance.md`),但**特異度很差**——它也會在明顯不是矢狀面的正面
  機位動作上吐出 `side`。同一個錯標還會悄悄把 `lunge_knee_valgus` 在那些動作上降級
  (`side` 不在 `ALIGNMENT_OBSERVABLE_VIEWS` 裡 → 可觀察度 `medium`、信心 ×0.65),但不改變
  它是否觸發。沒有任何閘門、門檻或信心下限因此被改動。
- **上面每一個列聯表數字都連同它的「結構性沉默」一起引用,因為不附這個的原始表格會誤導。**
  一下動作可能因為三個與指標無關的理由而無法觸發:**視角閘門**關著、遮罩到的階段短於
  `min_frames`(30fps 下是 6 幀)、或前導腿判不出來——而 `contingency` 會把每一下這種動作
  都算成真陰或偽陰,無形中壓低敏感度、灌高特異度。**這幾類是會重疊的,絕不能相加**——底下每
  一個數字都是聯集,並附上各分項與重疊數,好讓讀者自己加也對得起來。正式流程,各自出自 174 下:
  `lunge_knee_past_toes` **98 下不可動作**(86 下視角關閉 OR 32 下無法觸發,重疊 20 下),
  剩 76 下;`lunge_insufficient_depth` **48 下**(沒有視角關閉;其中 26 下是 `bottom` 階段
  短於下限),剩 126 下;`lunge_knee_valgus` **32 下**(沒有視角關閉),剩 142 下;
  `lunge_pelvic_drop` **111 下**(84 下視角關閉 OR 33 下無法觸發,重疊 6 下),剩 63 下。
  所以深度的「174 下觸發 6 次」與骨盆掉落的「174 下觸發 10 次」,有一部分是在講視窗長度與
  視角標籤,而不只是在講這個資料集裡有哪些錯誤。只取各規則真的能動作的那些動作所算的條件
  列聯表放在 `notes/lunge-rule-validation.md` §4,而它們也沒有救回這兩條規則。
- **兩項封住上述所有數字上限的量測條件。**(1)**影格有效率**:`lunge_compute_raw` 那道
  全有全無的關鍵點閘門,使得只有 **74.0%** 的 cam17 影格與 **58.4%** 的 cam18 影格帶得出任何
  指標——矢狀面那台更差,正如 `pushup.py` 對應註記所預測的。(2)**弓步計畫 §4.2 那句隔離
  宣稱,在實作上是錯的**:它說有了真實的動作邊界,`segment_reps` 就被完全繞過,但
  `run_detector` 對餵進去的任何東西都會自己做分段,結果它在 cam17 的 **174 下裡切了 152 下**、
  cam18 的 **174 下裡切了 91 下**。連續分數是就規則實際看到的那些影格計算的,所以分數的支撐
  集合與規則的支撐集合一致,而前導腿那項發現也另外在完整標註視窗上獨立重現過——但該規格承諾
  的隔離並沒有做到。要真的做到,得用 `replace(LUNGE_DETECTOR, rep_signal=None)`;本次沒有做。
