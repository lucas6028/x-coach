...
- **這次驗證真正沒過關的是前導腿的替代判準,而不是任何一個門檻——而且它框住了全部四條規則,**
  ...
  **但這個失敗首先是**量測**的失敗,其次才輪得到解剖學;因此文件刻意寫得比「彎曲較多無法辨識
  ...
  影像平面上計算、丟掉 MediaPipe 的偽深度 `z`,cam17 會從 **59.8% 掉到 17.2%**(獨立重算:
  14.8%),cam18 反而往上走。因此站得住的說法是:**「彎曲較多這條線索,以本管線量測它的方式,
...
- **`lunge_pelvic_drop`——幾乎沒被考到,而且 §6.5 預測的失效模式**並未被推翻**。** 它在 174 下
  ...
  算成真陰,而其中 37 下被視角關掉、18 下根本無法觸發**。只看**規則真的能動作的 24 下正確
...
- **上面每一個列聯表數字都連同它的「結構性沉默」一起引用,因為不附這個的原始表格會誤導。**
  ...
  都算成真陰或偽陰,無形中壓低敏感度、灌高特異度。正式流程的計數:`lunge_knee_past_toes`
  86 下視角關閉 + 32 下無法觸發;`lunge_insufficient_depth` 48 下無法觸發(其中 26 下是
  `bottom` 階段短於下限);`lunge_knee_valgus` 32 下;`lunge_pelvic_drop` 84 下視角關閉 + 33 下。
