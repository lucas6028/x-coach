...
**被 span 切到**：精修出的 window 貼齊 span 邊緣，**且該邊緣不是片子本身的邊緣** → 記
`refined: "clipped"`。這個值必須與 `false` 分開，不能併進去（見下方「已知風險」表：這是
`_validate_reps` 記一行 log 讓它在正式環境看得見的依據，混進 `false` 會讓那行 log 分不出來）。

「不是片子邊緣」這個條件不是細節：span 被片頭/片尾夾住時，外面根本沒有東西可抽，那不叫被切掉。
實測把兩者混為一談會report 43% 的 rep 被切，排除片緣後是 **1.4%**——與 §2.8 的 98.6% 涵蓋率吻合。

**精修必須用粗掃訊號導出的全片 band，不能讓每個 span 自己導百分位。**

「span 內重算百分位會讓帶變窄、進而移動邊界」這個疑慮**是真的**，而且第一次量的時候被自己的
量測方法藏了起來。那次先對全片平滑、再切片，等於從瀏覽器**根本沒抽取的 frame** 借了平滑上下文；
正式路徑只能在 span 之內平滑。改成與正式路徑一致之後（46 clips / 70 reps）：

| 精修的 band 來源 | 完全命中 | p95 | max |
|---|---|---|---|
| 每個 span 自己導出 | 92.9% | 15.3 幀 | 46 幀 |
| **粗掃訊號的全片 band（採用）** | **98.6%** | **0.0** | **1 幀** |

粗掃訊號涵蓋整支片子——那正是粗掃的用途——所以它的百分位在正式路徑上拿得到，不需要密集資料。
`segmentReps` 因此多一個選填的 band 覆寫參數（Python 的 `segment_reps` 同步增加，維持兩邊一致）。

**為什麼不接受 92.9%**：剩下 7% 的 rep 帶著最多 46 幀的邊界誤差，而 `assign_phases` 把區間的前
15% 當 setup。46 幀的偏移會讓 setup 落在下降段中間——正是 SP1 整份 spec 存在要修的那個 bug，
只是換了個路徑重新發生。這不是精度取捨，是同一個正確性問題。

**方法論教訓，值得留著**：第一次的量測用了正式路徑拿不到的資訊，於是把一個真實缺陷量成了 0。
量測若比它要認證的東西更寬裕，它認證的就不是那個東西。
...
1. **粗掃可能整個漏掉一下，或無中生有一下**（2/46 clips ≈ 4%：一個 0→1、一個 3→2）。漏掉的
   那一下不會被抽取，`reps.detected` 也會少報。UI（`Timeline.tsx` 的 `timeline.repsSummary`，
   見 §5）會照實顯示這個少報後的數字——「共 2 下，分析了第 1、2 下」——所以做了 3 下的使用者
   有機會發現數字不對。但這是**巧合，不是設計**：那行文字只列出偵測到的 rep 數與清單，並不會
   說「本來可能有一下沒被找到」，也不會指出**是哪一下**被漏掉。精修不解決這一項（它只在已知
   span 內重算）。這是 SP2 的已知代價，不是可以被 padding 蓋掉的東西。
...
`false` 分開，因為它是唯一能標出「`REP_PADDING_FRAMES` 不夠」的值（§2.8 量到 1.4% 的 rep 會落
在這裡）。

**這個欄位本身是唯寫的**：`RepWindow`（`src/pose/rep_segmentation.py`）沒有 `refined` 這個
屬性，`_validate_reps` 解析完 `segments` 之後這個值就不再往下傳——不進 log、不進資料庫、不進
UI。單靠這個值本身，不會有任何地方「看得見」。真正讓它在正式環境可見的，是
`_validate_reps` 在解析階段額外記的一行 INFO log（見下方「已知風險」表那一列的更正）。
...
| **粗掃整個漏掉一下，或無中生有一下**（實測 2/46 clips ≈ 4%） | 已知代價，無法用 padding 或精修修掉。漏掉的 rep 不會被抽取，`reps.detected` 也少報，`timeline.repsSummary` 會照實顯示少報後的數字（「共 2 下，分析了第 1、2 下」），讓做了 3 下的使用者有機會發現不對——**但這是巧合，不是設計**：那行文字不會說「可能漏了一下」，也不會指出是哪一下。若這個比例在真機上更糟，唯一的出路是提高粗掃 tier 或放棄粗掃改用即時 landmarks（§2.4），兩者都不在 SP2 |
| 假 rep 通過精修 | 一個假 rep 在精修時切不出 window，但那與「訊號太雜的真 rep」無法區分，所以只記 `refined: false`、**不丟棄**——丟棄會讓合法但雜訊大的 rep 靜靜消失 |
| `REP_PADDING_FRAMES = 24` 不夠，真區間被 span 切掉（實測 1.4%） | `refined: "clipped"` 標出這種 segment，但這個欄位本身**唯寫**——`_validate_reps` 解析完就不再傳遞它，不進 log、不進資料庫、不進 UI。實際可見性來自兩處：`_validate_reps` 額外記的一行 INFO log（`backend/app/routers/analyze.py`，任何 `analyzed` segment 帶 `"clipped"` 都會記）讓它在正式環境的伺服器 log 可查；§6 的真實語料測試會在比例惡化時在本機變紅 |
