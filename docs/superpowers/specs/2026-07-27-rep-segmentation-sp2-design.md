...
**95.7% 完全相同**（0 幀誤差，p95 = 0，max 27 就是那唯一一個被切到的 rep）；粗掃邊界是
p50 2 / p95 21 / max 45。「span 內重算百分位會讓帶變窄、進而移動邊界」是這個做法最明顯的疑慮，
實測上沒有發生。若哪天發生了，修法是把全片的 `lo`/`hi` 傳進精修而不是每個 span 各自導出。
