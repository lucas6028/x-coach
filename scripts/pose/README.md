...
# "Overhead Press", "Push-up", "Lunge" and "Row").
# (illustrative paths — no Overhead Press, Push-up, Lunge or Row dataset is checked in yet)
python scripts/pose/run_pose_rule_detection.py \
...
python scripts/pose/run_pose_rule_detection.py \
  --pose-json path/to/row_pose.json \
  --output-json results/single_detection.json \
  --movement "Row"
```
...
silently falling back. Currently `Squat`, `Overhead Press`, `Push-up`, `Lunge` and `Row` are
registered (only `Squat` is validated against labeled data; the rest, including `Lunge` and `Row`,
surface as Beta). Row ships 4 of its 5 spec'd rules — `rounded_thoracolumbar_spine` is a recorded
spec defect, not implemented (see
`docs/superpowers/specs/2026-07-18-16-movement-rule-detector-design.md` §7).
