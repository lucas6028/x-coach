...
| `20260927000000_daily_jobs.sql` | The nightly care-loop job (`POST /api/jobs/daily`, triggered by GitHub Actions): `daily_patient_reminders(since)` (linked LINE patients with no check-in on their current assigned plan since `since`), `daily_clinician_summaries(since, now)` (per clinician: patients, checked in today, open flags, 7-day inactive), `claim_job_run(job, date)` over a new `job_runs` table so one day cannot push twice, and `line_user_id_of(email)`. SECURITY DEFINER, **execute granted to `service_role` only**. The LINE user id comes from the synthetic login email, never from user-writable `user_metadata`. Needs `20260916000000_clinic.sql` first. Nothing else calls these, so an unmigrated database only breaks the job endpoint. |
