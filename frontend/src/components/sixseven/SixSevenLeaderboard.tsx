...
interface Props {
  entries: SixSevenEntry[];
  // 1-based rank to highlight (the score just submitted).
...
// Local top-10 board. Presentational — the caller loads/persists via lib/sixseven.
export default function SixSevenLeaderboard({ entries, highlightRank, compact }: Props) {
  const { t } = useI18n();
  ...
  if (entries.length === 0) {
    return (
      <div className="rounded-2xl border border-border-dark bg-surface-dark p-6 text-center">
        ...
        <p className="mt-2 text-sm text-muted">{t("six.board.empty")}</p>
      </div>
    );
  }
