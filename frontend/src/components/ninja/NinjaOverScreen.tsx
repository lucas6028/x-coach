...
export default function NinjaOverScreen({
  result,
  leaderboard,
  rank,
  submitted,
  ...
  const handleSubmit = () => {
    ...
    onSubmit(name.trim().slice(0, NAME_MAX) || t("ninja.over.anon"));
  };
      ...
      >
        <div className="rounded-3xl border border-border-dark bg-surface-dark p-7 text-center">
          ...
          <p className="mt-3 font-mono text-xs uppercase tracking-wider text-faint">
            {result.bombed ? t("ninja.over.bombed") : t("ninja.over.title")}
          </p>
          ...
          {!submitted ? (
            <div className="mt-6">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  maxLength={NAME_MAX}
                  placeholder={t("ninja.over.namePlaceholder")}
                  aria-label={t("ninja.over.nameLabel")}
            ...
            <p className="mt-5 text-sm font-medium text-primary">
              {rank && rank > 0 ? t("ninja.over.ranked", { rank }) : t("ninja.over.notRanked")}
            </p>
          )}
        </div>

        {submitted && (
          <NinjaLeaderboard entries={leaderboard} highlightRank={rank ?? undefined} />
        )}
